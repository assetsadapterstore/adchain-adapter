...
var (
	...
	PublicKeyPrefixCompat = "AD"

	MainnetPublic = addressEncoder.AddressType{"adchain", addressEncoder.BTCAlphabet, "ripemd160", "", 33, []byte(PublicKeyPrefixCompat), nil}
...
func (dec *AddressDecoderV2) AddressDecode(pubKey string, opts ...interface{}) ([]byte, error) {
	...
	} else if strings.HasPrefix(pubKey, PublicKeyPrefixCompat) { // "AD"
		pubKeyMaterial = pubKey[len(PublicKeyPrefixCompat):] // strip "AD"
	} else {
